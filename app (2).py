...
def extract_legal_issues(text):
    ...
    for i, para in enumerate(paragraphs):
        if re.search(r'(طعن|طلب|اعترض|رفض)', para) and "الطاعن" in para:
            طلب = para
            رد = ""
            قاعدة = ""
            for j in range(i+1, min(i+6, len(paragraphs))):
                if re.search(r'(قررت|رأت|خلصت|ردت المحكمة|حيث إنه)', paragraphs[j]):
                    رد = paragraphs[j]
                if re.search(r'(فإن القاعدة|مما سبق|لذلك|فقد استقر)', paragraphs[j]):
                    قاعدة = paragraphs[j]
            issues.append({"طلب": طلب, "رد": رد, "قاعدة": قاعدة})
    return issues if issues else [{"طلب": "لم يتم العثور على طلب واضح.", "رد": "-", "قاعدة": "-"}]

def create_word_file(issues, filename):
    doc = docx.Document()
    doc.add_heading('استخراج القواعد القضائية', level=1)
    for i, item in enumerate(issues, 1):
        doc.add_paragraph(f"{i}- {item['طلب']}", style='List Number')
        doc.add_paragraph(f"* رد المحكمة العليا: {item['رد']}")
        doc.add_paragraph(f"* القاعدة القضائية: {item['قاعدة']}")
        doc.add_paragraph("---")
    doc.save(filename)
...
if uploaded_files:
    if st.button("🔍 تحليل الأحكام وإنشاء ملف ZIP"):
